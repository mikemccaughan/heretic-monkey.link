 ...
 * @returns {number} A random value; probably within the Uint8 space.
 */
...
/**
 * Returns a value indicating whether a or b is greater, judging by its index.
 * @param {Cell} a The first cell to compare
 * @param {Cell} b The second cell to compare
 * @returns {number} A value greater than zero if a is greater than b, less than zero
 * if b is greater than a, or equal to zero if a is equal to b.
 */
...
export function generateBoard(width, height, density) {
  ...
  if (density <= 0 || density >= 1)
    throw new Error(`density must be > 0 and < 1, got ${density}`);
  /** @type { Cell[] } */
...
export function clearAround(clearAroundArgs) {
  const { board, index, onLose, onBlank, onNearby, onWin, onReveal } =
    clearAroundArgs;
...
export function revealCell(revealCellArgs) {
  if (revealCellArgs.onReveal) {
    revealCellArgs = revealCellArgs.onReveal(revealCellArgs);
  }
  revealCellArgs = {
    ...revealCellArgs,
    cells: updateCellAtIndex(
      revealCellArgs.cells,
      revealCellArgs.index,
      "hadOverlay",
      revealCellArgs.hadOverlay
    ),
  };
...
*/
/**
 * Toggles a flag indicating a cell possibly has a mine
 * @param {FlagCellArgs} flagCellArgs An object specifying the cell to (un)flag
