export class BoardBuilder {
         ...
         static createCell(x, y, value, index, clickCount, hidden, flag) {
           ...
           let key = `${x}.${y}.${value}`;
           return { key: key, value: value, x: x, y: y, index: index, clickCount: clickCount || 0, hidden: hidden, flag: flag };
         }
         static initializeBoard(width, height) {
           ...
           for (let y = 0; y < height; y++) {
             for (let x = 0; x < width; x++) {
               cells[cellIndex] = BoardBuilder.createCell(x, y, board[y][x], cellIndex);
               cellIndex++;
             }
           }
