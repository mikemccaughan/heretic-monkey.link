...
const HOUR_NANOS = BigInt(HOUR_SECONDS) * BILLION;
const MINUTE_NANOS_JSBI = SIXTY * BILLION;
const DAY_NANOS_JSBI = HOUR_NANOS * TWENTY_FOUR;
...
class TimeDuration {
  ...
  static fromEpochNsDiff(epochNs1, epochNs2) {
    const diff = ensureJSBI(epochNs1) - ensureJSBI(epochNs2);
  ...
  static fromComponents(h, min, s, ms, µs, ns) {
    const totalNs = BigInt(ns) + BigInt(µs) * THOUSAND + BigInt(ms) * MILLION + BigInt(s) * BILLION + BigInt(min) * MINUTE_NANOS_JSBI + BigInt(h) * HOUR_NANOS;
  ...
  add24HourDays(days) {
    ...
    return TimeDuration.validateNew(this.totalNs + BigInt(days) * DAY_NANOS_JSBI, 'sum');
  }
  addToEpochNs(epochNs) {
    return ensureJSBI(epochNs) + this.totalNs;
  }
  cmp(other) {
  ...
  fdiv(nParam) {
    ...
    const decimalDigits = [];
    let digit;
    const sign = (this.totalNs < ZERO ? -1 : 1) * Math.sign(Number(n));
    while (!(remainder === ZERO) && decimalDigits.length < precision) {
      remainder = remainder * TEN;
      ({
        quotient: digit,
        remainder
      } = divmod(remainder, nBigInt));
  ...
  round(incrementParam, mode) {
    ...
    const {
      quotient,
      remainder
    } = divmod(this.totalNs, increment);
    const sign = this.totalNs < ZERO ? 'negative' : 'positive';
    ...
    const rounded = remainder === ZERO ? r1 : ApplyUnsignedRoundingMode(r1, r2, cmp, isEven(quotient), unsignedRoundingMode);
...
const DATETIME_NS_MIN = NS_MIN - DAY_NANOS_JSBI + ONE;
const DATETIME_NS_MAX = NS_MAX + DAY_NANOS_JSBI - ONE;
...
const TEMPORAL_UNITS = [['years', 'year', 'date'], ['months', 'month', 'date'], ['weeks', 'week', 'date'], ['days', 'day', 'date'], ['hours', 'hour', 'time'], ['minutes', 'minute', 'time'], ['seconds', 'second', 'time'], ['milliseconds', 'millisecond', 'time'], ['microseconds', 'microsecond', 'time'], ['nanoseconds', 'nanosecond', 'time']];
...
function AdjustDateDurationRecord({
  years,
  months,
  weeks,
  days
}, newDays, newWeeks, newMonths) {
  return {
    years,
...
function InterpretISODateTimeOffset(isoDate, time, offsetBehaviour, offsetNs, timeZone, disambiguation, offsetOpt, matchMinute) {
  ...
  for (let index = 0; index < possibleEpochNs.length; index++) {
    ...
    const candidateOffset = Number(utcEpochNs - candidate);
...
function DisambiguatePossibleEpochNanoseconds(possibleEpochNs, timeZone, isoDateTime, disambiguation) {
  ...
  const dayBefore = utcns - DAY_NANOS_JSBI;
  ValidateEpochNanoseconds(dayBefore);
  const offsetBefore = GetOffsetNanosecondsFor(timeZone, dayBefore);
  const dayAfter = utcns + DAY_NANOS_JSBI;
...
const isTZIDSep = Object.assign(Object.create(null), {
  '/': true,
  '-': true,
  _: true
});
function GetAvailableNamedTimeZoneIdentifier(identifier) {
  ...
  const specialCases = new Map([['Act', 'ACT'], ['Lhi', 'LHI'], ['Nsw', 'NSW'], ['Dar_Es_Salaam', 'Dar_es_Salaam'], ['Port_Of_Spain', 'Port_of_Spain'], ['Port-Au-Prince', 'Port-au-Prince'], ['Isle_Of_Man', 'Isle_of_Man'], ['Comodrivadavia', 'ComodRivadavia'], ['Knox_In', 'Knox_IN'], ['Dumontdurville', 'DumontDUrville'], ['Mcmurdo', 'McMurdo'], ['Denoronha', 'DeNoronha'], ['Easterisland', 'EasterIsland'], ['Bajanorte', 'BajaNorte'], ['Bajasur', 'BajaSur']]);
...
function GetISOPartsFromEpoch(epochNanoseconds) {
  ...
  let nanos = Number(epochNanoseconds % MILLION);
  if (nanos < 0) {
    nanos += 1e6;
    epochMilliseconds -= 1;
  }
...
function GetNamedTimeZoneEpochNanoseconds(id, isoDateTime) {
  ...
  let nsEarlier = ns - DAY_NANOS_JSBI;
  if (nsEarlier < NS_MIN) nsEarlier = ns;
  let nsLater = ns + DAY_NANOS_JSBI;
...
function NudgeToCalendarUnit(sign, durationParam, destEpochNs, isoDateTime, timeZone, calendar, increment, unit, roundingMode) {
  ...
  // prettier-ignore
  const roundedUnit = numerator.isZero() ? Math.abs(r1) : !numerator.cmp(denominator) // equal?
  ...
  const fakeNumerator = new TimeDuration(denominator.totalNs * BigInt(r1) + numerator.totalNs * BigInt(increment * sign));
...
function DifferencePlainDateTimeWithTotal(isoDateTime1, isoDateTime2, calendar, unit) {
  ...
  if (unit === 'nanosecond') return Number(duration.time.totalNs);
  const destEpochNs = GetUTCEpochNanoseconds(isoDateTime2);
...
function RoundNumberToIncrementAsIfPositive(quantityParam, incrementParam, mode) {
  ...
  const cmp = compare(abs(remainder * TWO), increment) * (quantity < ZERO ? -1 : 1) + 0;
  const rounded = remainder === ZERO ? quotient : ApplyUnsignedRoundingMode(r1, r2, cmp, isEven(r1), unsignedRoundingMode);
...
function epochNsToMs(epochNanosecondsParam, mode) {
  ...
  let epochMilliseconds = Number(quotient);
  if (mode === 'floor' && Number(remainder) < 0) epochMilliseconds -= 1;
  if (mode === 'ceil' && Number(remainder) > 0) epochMilliseconds += 1;
...
function ToBigInt(arg) {
  ...
  if (typeof prim === 'bigint') {
    ...
    // an error.
    return BigInt(prim.toString(10));
  }
  ...
  isoToCalendarDate(isoDate, cache) {
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (this.hasEra && type === 'era' && value != null && value !== '') {
        ...
        result.era = value.split(' (')[0].normalize('NFD').replace(/[^-0-9 \p{L}]/gu, '').replace(/ /g, '-').toLowerCase();
      }
    }
...
class GregorianBaseHelper extends HelperBase {
  ...
  completeEraYear(calendarDate) {
    ...
    if (year != null) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
      const matchingEra = this.eras.find(({
        code,
        names = []
      }) => code === era || names.includes(era));
...
class JapaneseHelper extends SameMonthDayAsGregorianBaseHelper {
  ...
  reviseIntlEra(calendarDate, isoDate) {
    ...
    if (this.eras.find(e => e.code === era)) return {
      era,
      eraYear
    };
    return isoYear < 1 ? {
...
class NonIsoCalendar {
  ...
  resolveFields(fields /* , type */) {
    if (this.helper.calendarType !== 'lunisolar') {
      ...
      const largestMonth = this.helper.monthsInYear({
        year: fields.year ?? 1972
      }, cache);
...
class Instant {
  ...
  round(roundToParam) {
    ...
    const roundTo = typeof roundToParam === 'string' ? CreateOnePropObject('smallestUnit', roundToParam) : GetOptionsObject(roundToParam);
...
function getCalendarProperty$4(date, prop) {
  ...
  return calendarImplForObj(date).isoToDate(isoDate, {
    [prop]: true
  })[prop];
}
...
function getCalendarProperty$3(dt, prop) {
  ...
  return calendarImplForObj(dt).isoToDate(isoDate, {
    [prop]: true
  })[prop];
}
...
function getCalendarProperty$2(md, prop) {
  ...
  return calendarImplForObj(md).isoToDate(isoDate, {
    [prop]: true
  })[prop];
}
...
class PlainTime {
  ...
  with(temporalTimeLike, options = undefined) {
    ...
    let {
      ...
      microsecond,
      nanosecond
    } = Object.assign(fields, partialTime);
    const overflow = GetTemporalOverflowOption(GetOptionsObject(options));
    ({
      hour,
      minute,
...
function getCalendarProperty$1(ym, prop) {
  ...
  return calendarImplForObj(ym).isoToDate(isoDate, {
    [prop]: true
  })[prop];
}
...
class ZonedDateTime {
  ...
  equals(otherParam) {
    ...
    const one = GetSlot(this, EPOCHNANOSECONDS);
    const two = GetSlot(other, EPOCHNANOSECONDS);
    if (!(BigInt(one) === BigInt(two))) return false;
  ...
  getTimeZoneTransition(directionParam) {
    ...
    const direction = GetDirectionOption(typeof directionParam === 'string' ? CreateOnePropObject('direction', directionParam) : GetOptionsObject(directionParam));
...
function getCalendarProperty(zdt, prop) {
  ...
  return calendarImplForObj(zdt).isoToDate(isoDate, {
    [prop]: true
  })[prop];
}
...
const types = [Instant, PlainDate, PlainDateTime, Duration, PlainMonthDay,
